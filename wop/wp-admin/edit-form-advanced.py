...
user_ID = user_ID if isset(user_ID) else 0
action = action if isset(action) else ""
if post_type_supports(post_type, "editor")||post_type_supports(post_type, "thumbnail"):
  add_thickbox()
  wp_enqueue_media({"post":post_ID})
messages = []
messages["post"] = {0:"", 1:sprintf(__("Post updated. <a href="%s">View post</a>"), esc_url(get_permalink(post_ID))), 2:__("Custom field updated."), 3:__("Custom field deleted."), 4:__("Post updated."), 5:sprintf(__("Post restored to revision from %s"), wp_post_revision_title(_GET["revision"], False)) if isset(_GET["revision"]) else False, 6:sprintf(__("Post published. <a href="%s">View post</a>"), esc_url(get_permalink(post_ID))), 7:__("Post saved."), 8:sprintf(__("Post submitted. <a target="_blank" href="%s">Preview post</a>"), esc_url(add_query_arg("preview", "true", get_permalink(post_ID)))), 9:sprintf(__("Post scheduled for: <strong>%1$s</strong>. <a target="_blank" href="%2$s">Preview post</a>"), date_i18n(__("M j, Y @ G:i"), strtotime(post.post_date)), esc_url(get_permalink(post_ID))), 10:sprintf(__("Post draft updated. <a target="_blank" href="%s">Preview post</a>"), esc_url(add_query_arg("preview", "true", get_permalink(post_ID))))}
messages["page"] = {0:"", 1:sprintf(__("Page updated. <a href="%s">View page</a>"), esc_url(get_permalink(post_ID))), 2:__("Custom field updated."), 3:__("Custom field deleted."), 4:__("Page updated."), 5:sprintf(__("Page restored to revision from %s"), wp_post_revision_title(_GET["revision"], False)) if isset(_GET["revision"]) else False, 6:sprintf(__("Page published. <a href="%s">View page</a>"), esc_url(get_permalink(post_ID))), 7:__("Page saved."), 8:sprintf(__("Page submitted. <a target="_blank" href="%s">Preview page</a>"), esc_url(add_query_arg("preview", "true", get_permalink(post_ID)))), 9:sprintf(__("Page scheduled for: <strong>%1$s</strong>. <a target="_blank" href="%2$s">Preview page</a>"), date_i18n(__("M j, Y @ G:i"), strtotime(post.post_date)), esc_url(get_permalink(post_ID))), 10:sprintf(__("Page draft updated. <a target="_blank" href="%s">Preview page</a>"), esc_url(add_query_arg("preview", "true", get_permalink(post_ID))))}
...
if isset(_GET["message"]):
  ...
  elif !isset(messages[post_type])&&isset(messages["post"][_GET["message"]]):
...
if autosave&&mysql2date("U", autosave.post_modified_gmt, False)>mysql2date("U", post.post_modified_gmt, False):
  for autosave.autosave_field in autosave_field:
...
if "attachment"==post_type:
  ...
  add_meta_box("submitdiv", __("Save"), "attachment_submit_meta_box", , "side", "core")
  add_action("edit_form_after_title", "edit_form_image_editor")
else:
  add_meta_box("submitdiv", __("Publish"), "post_submit_meta_box", , "side", "core")
if current_theme_supports("post-formats")&&post_type_supports(post_type, "post-formats"):
  add_meta_box("formatdiv", _x("Format", "post format"), "post_format_meta_box", , "side", "core")
for tax_name in post:
  taxonomy = get_taxonomy(tax_name)
  if !taxonomy.show_ui:
  label = taxonomy.labels.name
  if !is_taxonomy_hierarchical(tax_name):
    add_meta_box("tagsdiv-"+tax_name, label, "post_tags_meta_box", , "side", "core", {"taxonomy":tax_name})
  else:
    add_meta_box(tax_name+"div", label, "post_categories_meta_box", , "side", "core", {"taxonomy":tax_name})
if post_type_supports(post_type, "page-attributes"):
  add_meta_box("pageparentdiv", __("Page Attributes") if "page"==post_type else __("Attributes"), "page_attributes_meta_box", , "side", "core")
if current_theme_supports("post-thumbnails", post_type)&&post_type_supports(post_type, "thumbnail"):
  add_meta_box("postimagediv", __("Featured Image"), "post_thumbnail_meta_box", , "side", "low")
if post_type_supports(post_type, "excerpt"):
  add_meta_box("postexcerpt", __("Excerpt"), "post_excerpt_meta_box", , "normal", "core")
if post_type_supports(post_type, "trackbacks"):
  add_meta_box("trackbacksdiv", __("Send Trackbacks"), "post_trackback_meta_box", , "normal", "core")
if post_type_supports(post_type, "custom-fields"):
  add_meta_box("postcustom", __("Custom Fields"), "post_custom_meta_box", , "normal", "core")
do_action("dbx_post_advanced")
if post_type_supports(post_type, "comments"):
  add_meta_box("commentstatusdiv", __("Discussion"), "post_comment_status_meta_box", , "normal", "core")
if "publish"==get_post_status(post)||"private"==get_post_status(post)&&post_type_supports(post_type, "comments"):
  add_meta_box("commentsdiv", __("Comments"), "post_comment_meta_box", , "normal", "core")
if !"pending"==get_post_status(post)&&!current_user_can(post_type_object.cap.publish_posts):
  add_meta_box("slugdiv", __("Slug"), "post_slug_meta_box", , "normal", "core")
if post_type_supports(post_type, "author"):
  if is_super_admin()||current_user_can(post_type_object.cap.edit_others_posts):
    add_meta_box("authordiv", __("Author"), "post_author_meta_box", , "normal", "core")
if post_type_supports(post_type, "revisions")&&0<post_ID&&wp_get_post_revisions(post_ID):
  add_meta_box("revisionsdiv", __("Revisions"), "post_revisions_meta_box", , "normal", "core")
...
print("</h2>\n")
if notice:
  print("<div id="notice" class="error"><p>")
  print(notice)
  print("</p></div>\n")
if message:
...
print(esc_attr(post.post_status))
print("" />\n<input type="hidden" id="referredby" name="referredby" value="")
print(esc_url(stripslashes(wp_get_referer())))
print("" />\n")
if !empty(active_post_lock):
  print("<input type="hidden" id="active_post_lock" value="")
  print(esc_attr(implode(":", active_post_lock)))
  print("" />\n")
...
if post_type_supports(post_type, "title"):
  ...
  print(apply_filters("enter_title_here", __("Enter title here"), post))
  print("</label>\n	<input type="text" name="post_title" size="30" value="")
  print(esc_attr(htmlspecialchars(post.post_title)))
  print("" id="title" autocomplete="off" />\n</div>\n<div class="inside">\n")
  ...
  if post_type_object.public&&!"pending"==get_post_status(post)&&!current_user_can(post_type_object.cap.publish_posts):
    print("	<div id="edit-slug-box" class="hide-if-no-js">\n	")
    if sample_permalink_html&&"auto-draft"!=post.post_status:
      print(sample_permalink_html)
    print("	</div>\n")
  print("</div>\n")
...
if post_type_supports(post_type, "editor"):
  print("<div id="postdivrich" class="postarea">\n\n")
  wp_editor(post.post_content, "content", {"dfw":True, "tabfocus_elements":"sample-permalink,post-preview", "editor_height":360})
